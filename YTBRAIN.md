...
Превращает историю просмотров YouTube в архив конспектов и подробную
статистику: динамика по месяцам/часам/дням недели, тепловая карта,
типы записей (видео / посты / реклама / удалённые), все каналы с оценкой
времени, самые пересматриваемые и «самые долгие» видео, сессии-марафоны,
история поиска. Всё живёт в одном репозитории: GitHub Actions
обрабатывает данные по расписанию, GitHub Pages показывает дашборд.

Файл истории можно **загрузить прямо на странице дашборда** — он
анализируется локально в браузере и никуда не отправляется. Постоянное
хранение — через `data/incoming/` (см. ниже).
...
- **YouTube блокирует IP-адреса GitHub Actions** («RequestBlocked») —
  субтитры из CI чаще всего не выкачиваются. Запускайте
  `python scripts/summarize.py` локально со своего IP и пушьте
  результат; статистика от этого не зависит и собирается в CI всегда.
- **Время просмотра — оценка**: Takeout не хранит длительность, поэтому
  она считается по интервалам между соседними событиями (интервалы
  больше 40 минут не учитываются). Помечено знаком ≈.
